...
"""Api built for authentication using jwt  tokens
    api built for following endpoints
    login
    logut
    register
    otp verification
...
class VerifyOTP(APIView):
    def post(self, request):
        try:
            ...
            return Response(
                {"status": 400, "message": "Invalid data", "errors": serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                ...
                if user is None:
                    return Response(
                        {"status": 400, "message": "invalid password", "data": {}},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
